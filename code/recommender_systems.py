...
def most_similar_users_to(user_id):
    ...
    return sorted(pairs,                                      # sort them
                  key=lambda __similarity: __similarity[1],     # most similar
                  reverse=True)                               # first
...
def user_based_suggestions(user_id, include_current_interests=False):
    ...
    suggestions = sorted(list(suggestions.items()),
                         key=lambda __weight: __weight[1],
                         reverse=True)
...
def most_similar_interests_to(interest_id):
    ...
    return sorted(pairs,
                  key=lambda __similarity1: __similarity1[1],
                  reverse=True)
...
def item_based_suggestions(user_id, include_current_interests=False):
    ...
    suggestions = sorted(list(suggestions.items()),
                         key=lambda __similarity2: __similarity2[1],
                         reverse=True)
